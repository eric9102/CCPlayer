...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	if (filePath && [[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:&isDir] && !isDir)
	{
        ...
        NSLog(@"\n[Function]:%s\n" "[line]:%d\n" "[value]:%@\n",__FUNCTION__, __LINE__, @"hahahhahahah");
        
        NSRange range = [[[request url] absoluteString] rangeOfString:@"/" options:NSBackwardsSearch];
        NSString * suffix = [[[request url] absoluteString] substringFromIndex:range.location+1];
        NSLog(@"\n[Function]:%s\n" "[line]:%d\n" "[value]:%@\n",__FUNCTION__, __LINE__, suffix);
        
//        NSString *DocumentPath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:@"hls-ecb-test"];
//        NSString *keyPath = [DocumentPath stringByAppendingPathComponent:@"file.key"];
//        NSData *keyData = [NSData dataWithContentsOfFile:keyPath];
//        NSString *key = [keyData convertDataToHexStr];
        
        NSData *videoData = [NSData dataWithContentsOfFile:filePath];
        NSData *deData = [videoData AES128DecryptWithKey:@"123"];
        
        return [[HTTPDataResponse alloc] initWithData:deData];
